		...
		<div>
						...
						<div className="px-4 pt-4 sm:px-8 md:px-16">
							<div className="text-center">
								...
								<button
									...
									type="button"
									className="py-2 px-4 m-4 bg-primary hover:bg-secondary active:bg-secondary focus:outline-none focus:ring focus:ring-white text-white transition ease-in duration-200 text-center text-base font-semibold shadow-md rounded-lg md:w-56"
								>
									Add to cart
								</button>
							</div>
							<div>
								...
								<p className="text-gray-300 whitespace-pre-line">{selected.product.description}</p>
							</div>
						</div>
					</div>
				)}
			...
			{Object.keys(props.stripe_products).map((category) => (
				<div key={category} className="container mx-auto my-8 px-4 sm:px-6 lg:px-8 xl:px-24">
					...
					<Carousel arrows={!selected} swipeable={true} draggable={true} showDots={true} keyBoardControl={true} responsive={responsive}>
					...
					{cart !== '{}' ? (
						<>
							...
							<Link href={{ pathname: '/shop/products/checkout' }} passHref legacyBehavior>
								<button
									type="button"
									className="w-96 px-3 py-3 text-sm shadow rounded-lg text-white bg-primary hover:bg-secondary transition ease-in duration-200"
								>
									Checkout
								</button>
							</Link>
...
export async function getStaticProps() {
	// if Stripe is unreachable at build time, build an empty store;
	...
	return {
		props: { stripe_products: stripe_products },
		// note: this used to be nested inside props, so the store never actually revalidated
		revalidate: 3600
	};
}
