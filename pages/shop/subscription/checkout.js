		...
		<div>
			...
			<div className="flex flex-wrap justify-center text-white text-center text-l bg-dark p-8">
				<div className="text-left w-96 relative">
					<p>
						Cost: {price} / {annual === 'true' ? 'year' : 'month'}{' '}
					</p>
					...
					<input
						...
						id="email"
						disabled={true}
						className="opacity-50 rounded-lg border-transparent flex-1 appearance-none border border-gray-300 w-full py-2 px-4 bg-white text-gray-700 placeholder-gray-400 shadow-sm text-base"
						name="email"
					...
					<div className="h-4" />
					{mcErrorMessage !== '' && (
						<label htmlFor="validate-username" className="text-red-500">
							{mcErrorMessage}
						</label>
					)}
					<button
						...
						disabled={validating}
						onClick={() => validate_user(setCheckout, setValidating, setMcErrormessage, setUUID)}
						className="flex justify-center items-center w-l py-2 px-4  bg-primary hover:bg-secondary text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md rounded-lg"
					>
						{validating ? (
							<>
								<Spinner /> <p>Loading...</p>
					...
					{!loading && canCheckout && (
						<>
							...
							<div className="rounded-lg border-2 border-primary flex flex-wrap justify-center p-8">
								<Image
									id="mc-skin-preview"
									src={'https://mc-heads.net/body/' + uuid + '/right/'}
									height={432}
									width={180}
									draggable={false}
									unoptimized={true}
							...
							<label className="flex items-center space-x-3 mb-3">
								...
								/>
								<span className="text-white">This is the correct Minecraft account</span>
							</label>
							<label className="flex items-center space-x-3 mb-3">
								<input
									type="checkbox"
									onChange={() => setBox2(!box2)}
									name="terms-check"
									className="form-tick appearance-none bg-white bg-check h-6 w-6 border border-gray-300 rounded-md checked:bg-green-500 checked:border-transparent focus:outline-none"
								/>
							...
							{stripeErrorMessage !== '' && (
								<label htmlFor="checkout" className="text-red-500">
									{stripeErrorMessage}
								</label>
							)}
							<button
								type="button"
								id="checkout"
								onClick={() => load_stripe(price_id, box1 && box2, setStripeLoading, setStripeErrorMessage)}
								className={`flex justify-center items-center w-l py-2 px-4 text-white w-full transition ease-in duration-200 text-center text-base font-semibold shadow-md rounded-lg ${box1 && box2 ? 'bg-primary hover:bg-secondary' : 'bg-light cursor-not-allowed'}`}
							>
								{stripeLoading ? (
									<>
										<Spinner /> <p>Loading...</p>
...
function validate_user(setCheckout, setValidating, setErrormessage, setUUID) {
	...
	uuid_request.open('get', '/api/minecraft/getuuid/' + username);
...
function checkout(sub, setLoading, setErrormessage) {
	...
	checkout_request.open(
		'get',
		'/api/subscribe/' +
			sub +
			'?user=' +
			encodeURIComponent(document.getElementById('mc-username').value) +
			'&uuid=' +
			encodeURIComponent(document.getElementById('mc-uuid').value)
	);
