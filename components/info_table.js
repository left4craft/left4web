...
export function InfoTable({ data, success, type }) {
				...
				<table className="table-auto w-11/12 max-w-lg border-4 border-primary">
					<tbody>
							...
							<tr className="hover:bg-light cursor-pointer">
								...
								<td className="border border-light px-4 py-2">
									<div className="flex items-center">
										<Image
											src={`/api/minecraft/gethead/${data.result.uuid}`}
											height={32}
											width={32}
											...
											unoptimized
										/>
										<div className="px-2">{data.result.name}</div>
									</div>
								</td>
							</tr>
						</Link>
						<Link href={`/punishments/by/${data.result.banned_by_uuid}`} passHref legacyBehavior>
							<tr className="hover:bg-light cursor-pointer">
								<td className="border border-light px-4 py-2">{action} by</td>
								<td className="border border-light px-4 py-2">
									<div className="flex items-center">
										<Image
											src={data.result.banned_by_uuid === 'CONSOLE' ? '/images/console.png' : `/api/minecraft/gethead/${data.result.banned_by_uuid}`}
											height={32}
											width={32}
											className="pixelated rounded-md"
											draggable={false}
											unoptimized
										/>
										<div className="px-2">{data.result.banned_by}</div>
									</div>
								</td>
							</tr>
						</Link>
						...
						<tr className="hover:bg-light">
							...
							<td className="border border-light px-4 py-2">
								{data.result.until <= 0 ? 'Permanent' : new Date(data.result.until).toLocaleString() + removed_by}
							</td>
						</tr>
						<tr className="hover:bg-light">
