...
export function ListTable({ data, success, type }) {
	...
	return (
		<>
			<div className="h-12 flex items-center text-white border-b border-primary text-base font-medium">
				<Link href="/punishments/bans/1" scroll={false} passHref legacyBehavior>
					<button
						type="button"
						className={`w-full rounded-tl-lg h-full px-4 transition ease-in duration-200 focus:outline-none ${type[1] === 'bans' ? 'bg-primary' : 'bg-light'} hover:bg-secondary`}
					>
						Bans
					</button>
				</Link>
				<Link href="/punishments/mutes/1" scroll={false} passHref legacyBehavior>
					<button
						type="button"
						className={`w-full h-full px-4 transition ease-in duration-200 focus:outline-none ${type[1] === 'mutes' ? 'bg-primary' : 'bg-light'} hover:bg-secondary`}
					>
						Mutes
					</button>
				</Link>
				<Link href="/punishments/kicks/1" scroll={false} passHref legacyBehavior>
					<button
						type="button"
						className={`w-full h-full px-4 transition ease-in duration-200 focus:outline-none ${type[1] === 'kicks' ? 'bg-primary' : 'bg-light'} hover:bg-secondary`}
					>
						Kicks
					</button>
				</Link>
				<Link href="/punishments/warnings/1" scroll={false} passHref legacyBehavior>
					<button
						type="button"
						className={`w-full rounded-tr-lg h-full px-4 transition ease-in duration-200 focus:outline-none ${type[1] === 'warnings' ? 'bg-primary' : 'bg-light'} hover:bg-secondary`}
					>
						Warnings
					</button>
				</Link>
		...
		`}</style>
			<table className="min-w-full border-collapse block md:table border-primary border-4">
				...
				<tbody className="block md:table-row-group text-white">{rows}</tbody>
			</table>
			<div className="flex items-center text-white h-12">
				<Link href={`/punishments/${type[1]}/${Math.max(1, data.pragnation.page)}`} scroll={false} passHref legacyBehavior>
					<button
						type="button"
						className="w-full text-base font-medium rounded-bl-lg bg-light hover:bg-secondary h-full px-4 focus:outline-none transition ease-in duration-200"
					>
						Previous Page
					</button>
				</Link>
				...
				>
					<button
						type="button"
						className="w-full text-base font-medium rounded-br-lg bg-light hover:bg-secondary h-full px-4 focus:outline-none transition ease-in duration-200"
					>
						Next Page
					</button>
				</Link>
...
function ListRow({ action, rowData, type }) {
	let removed_by = '';
		...
		<Link href={`/punishments/${type}/info/${rowData.id}`} passHref legacyBehavior>
			<tr className="bg-dark cursor-pointer hover:bg-light border border-light md:border-none block md:table-row">
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">Player</span>
					<span className="md:hidden">{rowData.name}</span>
					<div className="items-center hidden md:flex">
						<div className="mr-2">
							<Image
								src={`/api/minecraft/gethead/${rowData.uuid}`}
								height={32}
								width={32}
				...
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">{`${action} by`}</span>
					<span className="md:hidden">{rowData.banned_by}</span>
					<div className="items-center hidden md:flex">
						<div className="mr-2">
							<Image
								src={rowData.banned_by_uuid === 'CONSOLE' ? '/images/console.png' : `/api/minecraft/gethead/${rowData.banned_by_uuid}`}
								height={32}
								width={32}
								className="pixelated rounded-md"
								draggable={false}
				...
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">Reason</span>
					{rowData.reason}
				</td>
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">Date</span>
					{new Date(rowData.time).toLocaleString()}
				</td>
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">Expires</span>
					{(rowData.until <= 0 ? 'Permanent' : new Date(rowData.until).toLocaleString()) + removed_by}
				</td>
			</tr>
		</Link>
	);
