...
export function SearchBar({ setErrorText }) {
	function search(mobile) {
		...
		query_request.open('get', '/api/punishments/check?name=' + encodeURIComponent(query));
	...
	return (
		<>
			<div className="text-white md:hidden block space-y-4">
				<div className="flex justify-center">
					<select id="search-type-mobile" className="h-8 border border-primary pl-2 pr-7 bg-light rounded-lg focus:outline-none">
						<option value={'for'}>Punishments for</option>
						<option value={'by'}>Punishments by</option>
					</select>
				</div>
				<div className="flex justify-center">
					<input
						...
						className="h-8 border border-primary pl-2 pr-7 bg-light rounded-lg focus:outline-none"
					/>
				</div>
				<div className="flex justify-center">
					<button
						...
						onClick={() => search(true)}
						type="button"
						className="h-8 py-2 px-4 border border-primary pl-2 pr-7 bg-light rounded-lg focus:outline-none hover:bg-secondary transition ease-in duration-200"
					>
						Search
					</button>
				</div>
			...
			<div className="text-white hidden md:flex justify-center">
				<select
					id="search-type"
					className="py-2 px-4 border-t border-l border-b border-primary  h-full pl-2 pr-7 bg-light rounded-l-lg focus:outline-none"
				>
					<option value={'for'}>Punishments for</option>
					<option value={'by'}>Punishments by</option>
				</select>
				<input
					...
					type="search"
					placeholder="Username or UUID"
					className="py-2 px-4 border-t border-b border-primary h-full pl-2 pr-7 bg-light focus:outline-none"
				/>
				<button
					type="button"
					onClick={() => search(false)}
					id="search"
					className="py-2 px-4 border-t border-r border-b border-primary h-full pl-2 pr-7 bg-light rounded-r-lg focus:outline-none hover:bg-secondary transition ease-in duration-200"
				>
					Search
				</button>
			</div>
