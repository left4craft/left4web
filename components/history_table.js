...
export function HistoryTable({ data, success, type }) {
	...
	return (
		<>
			<div className="rounded-t-lg h-4 flex bg-primary items-center text-white border-b border-primary text-base font-medium"></div>
			<style>{`
		...
		`}</style>
			...
			<div className="flex items-center text-white h-12">
				<Link href={`/punishments/${type[1]}/${data.minecraft.uuid}/${Math.max(1, data.pragnation.page)}`} scroll={false} passHref legacyBehavior>
					<button
						type="button"
...
function HistoryRow({ action, rowData }) {
	let removed_by = '';
	...
	return (
		<Link href={`/punishments/${rowData.type}/info/${rowData.id}`} passHref legacyBehavior>
			<tr className="bg-dark cursor-pointer hover:bg-light border border-light md:border-none block md:table-row">
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
					<span className="inline-block w-1/3 md:hidden font-bold">Type</span>
					{type}
				</td>
				<td className="p-2 md:border md:border-light text-left block md:table-cell">
